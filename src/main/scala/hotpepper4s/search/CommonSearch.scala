...
import hotpepper4s.{EntireImpl, Entire, Results, HotPepper}
import org.json4s._
 ...
 */
class CommonSearch[A, B <: Results[A]](apiName: String)(implicit val mf: Manifest[B]) {
  ...
  def list(qMap: Map[String, String]): List[A] = {
    ...
    json.extract[EntireImpl[B]].results.data
  }
